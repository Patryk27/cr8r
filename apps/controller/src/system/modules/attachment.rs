...
impl Attachment {
    ...
    pub async fn add_chunk(&self, chunk: Vec<u8>) -> Result<()> {
        ask!(self.tx, AttachmentMsg::UploadChunk { chunk })
    }
    ...
    pub async fn download(&self) -> Result<AttachmentDownloadToken> {
        ask!(self.tx, AttachmentMsg::Download)
    }
