...
use crate::system::{Experiment, SystemEvent};
...
pub async fn launch(actor: &mut ExperimentStoreActor, definition: DDefinition) -> Result<DExperimentId> {
    ...
    for dep in &definition.dependencies {
        if let DDependencySourceDef::Path { attachment_id } = &dep.source {
            let attachment = actor.attachment_store.find_one(*attachment_id).await?;
    ...
    actor.bus.emit(SystemEvent::ExperimentCreated { id });
