...
use crate::backend::{Compiler, Experiment, Result, System, SystemCommand, SystemCommandRx};
...
impl SystemActor {
        ...
        while let Some(cmd) = rx.next().await {
            ...
            match cmd {
                SystemCommand::RequestAssignment { runner, tx } => {
                    let _ = tx.send(
                        self.do_request_assignment(runner).await,
                    );
                }

                SystemCommand::FindExperiment { experiment, tx } => {
                    let _ = tx.send(
                        self.do_find_experiment(experiment).await,
                    );
                }

                SystemCommand::LaunchExperiment { experiment, tx } => {
                    let _ = tx.send(
                        self.do_launch_experiment(experiment).await,
                    );
                }

                SystemCommand::RegisterRunner { name, secret, tx } => {
                    let _ = tx.send(
                        self.do_register_runner(name, secret).await,
                    );
                }
            };
        }

        debug!("System actor has been orphaned, halting it");
    }
    ...
    async fn do_find_experiment(&mut self, experiment: ExperimentId) -> Result<Experiment> {
        self.experiments
            .find_by_id(&experiment)
            .map(ToOwned::to_owned)
    }

    async fn do_launch_experiment(&mut self, definition: ExperimentDefinitionInner) -> Result<ExperimentId> {
    ...
    async fn do_register_runner(&mut self, name: RunnerName, secret: RunnerSecret) -> Result<RunnerId> {
