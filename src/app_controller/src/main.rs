...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let system_addr = modules::SystemActor::new(config.controller.runner_secret, compiler)
        .start();

    // Launch the HTTP server actor (it's a non-blocking call)
    http::Server::new(&config.controller.bind, system_addr)
        .start()?;
